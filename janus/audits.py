...
def attack_surface(
    ...
    flipped_ids = sample.loc[flipped, "applicant_id"].tolist()
...
def evidence_recourse(holdout: pd.DataFrame, scorecard: Scorecard) -> dict:
    declined = _declined(holdout, scorecard)
    informal = declined.loc[declined["is_informal"] == 1].copy()
    shares = (0.25, 0.50, 0.75, 1.00)
    curve = []
    X = informal[FEATURES].copy()
...
def proxy_audit(holdout: pd.DataFrame) -> dict:
    y = holdout["is_rural"].to_numpy()
    X = holdout[FEATURES]
    pipe = Pipeline(
        [
    ...
    ranked = sorted(zip(FEATURES, coef), key=lambda kv: abs(kv[1]), reverse=True)
    return {
...
def discover_broken_segments(holdout: pd.DataFrame, scorecard: Scorecard, min_leaf: int = 120) -> dict:
    p = scorecard.predict_proba(holdout)
    # Residual: actual default minus predicted. Tree on features → residual sign.
    residual = holdout["default"].to_numpy() - p
    y = (residual > 0.08).astype(int)  # understated risk
    tree = DecisionTreeClassifier(max_depth=3, min_samples_leaf=min_leaf, random_state=7)
    tree.fit(holdout[FEATURES], y)
    leaf = tree.apply(holdout[FEATURES])
    rows = []
    for lid in np.unique(leaf):
        mask = leaf == lid
        if mask.sum() < min_leaf:
            continue
    ...
    return {
        ...
        "tree": export_text(tree, feature_names=FEATURES, max_depth=3),
        "leaves": rows[:8],
        ...
        "groups": [
            {
                "group": name,
                "features": feats,
                "abs_coefficient_share": round(sum(abs(coef[f]) for f in feats) / total, 4),
            }
...
def recourse_menu(
    ...
    # Route C — document existing income.
    trial = pd.DataFrame([row])[FEATURES].copy()
    trial["debt_to_income"] = documented_dti(
